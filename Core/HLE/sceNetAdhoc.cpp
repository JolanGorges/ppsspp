...
static void __AdhocctlNotify(u64 userdata, int cyclesLate)
{
	...
	if (g_Config.bEnableWlan)
	{
		...
		if (len > 0)
		{
			...
			{
				DEBUG_LOG(Log::sceNet, "sceNetAdhocctl[%i]: Sending OPCODE %d", uid, req.opcode);
...
void netAdhocValidateLoopMemory()
{
	...
	if (!matchingThreadHackAddr || (matchingThreadHackAddr && strcmp("matchingThreadHack", kernelMemory.GetBlockTag(matchingThreadHackAddr)) != 0))
	{
		...
		if (matchingThreadHackAddr)
			Memory::Memcpy(matchingThreadHackAddr, matchingThreadCode, sizeof(matchingThreadCode));
	}
}
...
u32 sceNetAdhocInit()
{
	...
	if (!netInited)
		return hleLogError(Log::sceNet, SCE_KERNEL_ERROR_LWMUTEX_NOT_FOUND, "net was not initialized?");

	// Already initialized
	if (netAdhocInited)
		return hleLogError(Log::sceNet, ERROR_NET_ADHOC_ALREADY_INITIALIZED, "already initialized");
...
static int sceNetAdhocPtpAccept(int id, u32 peerMacAddrPtr, u32 peerPortPtr, int timeout, int flag)
{
	...
	if (netAdhocInited)
	{
		...
		if (true)
		{ // FIXME: Not sure what kind of arguments considered as invalid (need to be tested on a homebrew), might be the flag?
			// Valid Socket
			if (id > 0 && id <= MAX_SOCKET && adhocSockets[id - 1] != NULL)
			{
				// Cast Socket
...
static int sceNetAdhocMatchingCreate(int mode, int maxnum, int port, int rxbuflen, int hello_int, int keepalive_int, int init_count, int rexmt_int, u32 callbackAddr)
{
	...
	if (netAdhocMatchingInited)
	{
		...
		if (maxnum > 1 && maxnum <= PSP_NET_ADHOC_MATCHING_MAXNUM)
		{
			...
			if (rxbuflen >= 1)
			{ // 1024 //200 on DBZ Shin Budokai 2
				// Valid Arguments
					...
					if (context != NULL)
					{
						...
						if (context->rxbuf != NULL)
						{
							...
							if (keepalive_int < 1)
								...
								context->keepalive_int = keepalive_int; // client might set this to 0
			...
			if (context != NULL)
			{
				...
				if (context->running)
				{
					...
					if (peer != NULL)
					{
						...
						{
							void *opt = NULL;
							if (Memory::IsValidAddress(optDataPtr))
								opt = Memory::GetPointerWriteUnchecked(optDataPtr);
							// Host Mode
							if (context->mode == PSP_ADHOC_MATCHING_MODE_PARENT)
							{
								// Already Connected
								if (peer->state == PSP_ADHOC_MATCHING_PEER_CHILD)
									return hleLogError(Log::sceNet, ERROR_NET_ADHOC_MATCHING_ALREADY_ESTABLISHED, "adhocmatching already established");

								// Not enough space
								if (countChildren(context) == (context->maxpeers - 1))
									return hleLogError(Log::sceNet, ERROR_NET_ADHOC_MATCHING_EXCEED_MAXNUM, "adhocmatching exceed maxnum");
							...
							else if (context->mode == PSP_ADHOC_MATCHING_MODE_CHILD)
							{
								...
								if (findOutgoingRequest(context) != NULL)
									return hleLogError(Log::sceNet, ERROR_NET_ADHOC_MATCHING_REQUEST_IN_PROGRESS, "adhocmatching request in progress");
...
static int sceNetAdhocMatchingGetMembers(int matchingId, u32 sizeAddr, u32 buf)
{
	...
	{
		WARN_LOG(Log::sceNet, "MemberList: Prev (%u) ==> Current (%u)", prev_members / sizeof(SceNetAdhocMatchingMemberInfoEmu), *buflen / sizeof(SceNetAdhocMatchingMemberInfoEmu));
	}
	prev_members = *buflen;
			...
			if (context != NULL)
			{
				...
				if (context->running)
				{
					...
					if (Memory::IsValidAddress(dataAddr))
						data = Memory::GetPointerWriteUnchecked(dataAddr);
...
static int sceNetAdhocMatchingGetPoolMaxAlloc()
{
	...
	return hleLogDebug(Log::sceNet, netAdhocPoolStat.maximum, "faked value");
}
...
void __NetMatchingCallbacks() //(int matchingId)
{
	...
	if (params != matchingEvents.end())
	{
		...
		{
			DEBUG_LOG(Log::sceNet, "AdhocMatchingCallback: [ID=%i][EVENT=%i][%s]", ctxid, eventid, mac2str((SceNetEtherAddr *)Memory::GetPointer(bufAddr)).c_str());
		...
		{
			DEBUG_LOG(Log::sceNet, "AdhocMatching - Discarding Callback: [ID=%i][EVENT=%i][%s]", ctxid, eventid, mac2str((SceNetEtherAddr *)Memory::GetPointer(bufAddr)).c_str());
			matchingEvents.pop_front();
		}
	}
...
static int sceNetAdhocctlGetPeerList(u32 sizeAddr, u32 bufAddr)
{
	...
	if (netAdhocctlInited)
	{
		...
		if (buflen != NULL)
		{
			...
			{
				WARN_LOG(Log::sceNet, "PeerList: Prev (%u) ==> Current (%u)", prev_peers / sizeof(SceNetAdhocctlPeerInfoEmu), *buflen / sizeof(SceNetAdhocctlPeerInfoEmu));
			}
			prev_peers = *buflen;
...
void sendAcceptPacket(SceNetAdhocMatchingContext *context, SceNetEtherAddr *mac, int optlen, void *opt)
{
	...
	if (peer != NULL && (peer->state == PSP_ADHOC_MATCHING_PEER_CHILD || peer->state == PSP_ADHOC_MATCHING_PEER_P2P))
	{
		...
		// Parent Mode
		if (context->mode == PSP_ADHOC_MATCHING_MODE_PARENT)
			siblingbuflen = (u32)sizeof(SceNetEtherAddr) * (countConnectedPeers(context) - 2);
...
void actOnHelloPacket(SceNetAdhocMatchingContext *context, SceNetEtherAddr *sendermac, int32_t length)
{
	...
	if ((context->mode == PSP_ADHOC_MATCHING_MODE_CHILD && findParent(context) == NULL) || (context->mode == PSP_ADHOC_MATCHING_MODE_P2P && findP2P(context) == NULL))
	{
		...
		if (length >= 5)
		{
			...
			int optlen = 0;
			memcpy(&optlen, context->rxbuf + 1, sizeof(optlen));
	...
	if (context->mode != PSP_ADHOC_MATCHING_MODE_PARENT)
	{
		...
		if ((context->mode == PSP_ADHOC_MATCHING_MODE_CHILD && findParent(context) == NULL) || (context->mode == PSP_ADHOC_MATCHING_MODE_P2P && findP2P(context) == NULL))
		{
			...
			if (length >= 9)
			{
				...
				int siblingcount = 0;
				memcpy(&siblingcount, context->rxbuf + 5, sizeof(siblingcount));
				...
				if (optlen >= 0 && length >= (9LL + optlen + static_cast<long long>(sizeof(SceNetEtherAddr)) * siblingcount))
				{
					...
					if (siblingcount > 0)
						siblings = (SceNetEtherAddr *)(context->rxbuf + 9 + optlen);
...
void actOnBulkDataPacket(SceNetAdhocMatchingContext *context, SceNetEtherAddr *sendermac, int32_t length)
{
	...
	// Established Peer
	if (peer != NULL && ((context->mode == PSP_ADHOC_MATCHING_MODE_PARENT && peer->state == PSP_ADHOC_MATCHING_PEER_CHILD) ||
						 ...
						 (context->mode == PSP_ADHOC_MATCHING_MODE_P2P && peer->state == PSP_ADHOC_MATCHING_PEER_P2P)))
	{
		...
		if (length > 5)
		{
			// Extract Data Length
			int datalen = 0;
			memcpy(&datalen, context->rxbuf + 1, sizeof(datalen));
		...
		while (contexts != NULL && context->eventRunning)
		{
			...
			while (context != NULL && context->event_stack != NULL)
			{
				...
				if (msg != NULL)
				{
					...
					// Grab Optional Data
					if (msg->optlen > 0)
						opt = ((u8 *)msg) + sizeof(ThreadMessage); //&msg[1]
			...
			while (context != NULL && context->inputRunning && !Core_IsStepping())
			{
				...
				if (context->input_stack != NULL)
				{
					...
					while (msg != NULL)
					{
						...
						// Grab Optional Data
						if (msg->optlen > 0)
							opt = ((u8 *)msg) + sizeof(ThreadMessage);

						// context->inputlock->unlock(); // Unlock to prevent race condition when locking peerlock

						// Send Accept Packet
						if (msg->opcode == PSP_ADHOC_MATCHING_PACKET_ACCEPT)
							sendAcceptPacket(context, &msg->mac, msg->optlen, opt);

						// Send Join Packet
						else if (msg->opcode == PSP_ADHOC_MATCHING_PACKET_JOIN)
							sendJoinPacket(context, &msg->mac, msg->optlen, opt);

						// Send Cancel Packet
						else if (msg->opcode == PSP_ADHOC_MATCHING_PACKET_CANCEL)
							sendCancelPacket(context, &msg->mac, msg->optlen, opt);

						// Send Bulk Data Packet
						else if (msg->opcode == PSP_ADHOC_MATCHING_PACKET_BULK)
							sendBulkDataPacket(context, &msg->mac, msg->optlen, opt);

						// Send Birth Packet
						else if (msg->opcode == PSP_ADHOC_MATCHING_PACKET_BIRTH)
							sendBirthPacket(context, &msg->mac);

						// Send Death Packet
						else if (msg->opcode == PSP_ADHOC_MATCHING_PACKET_DEATH)
							sendDeathPacket(context, &msg->mac);
				...
				if (recvresult == 0 && rxbuflen > 0)
				{
					...
					if (peer != NULL)
					{
						...
						if (peer->last_recv != 0)
							peer->last_recv = std::max(peer->last_recv, now - defaultLastRecvDelta);
					}
					else
					{
					...
					// Ping Packet
					if (context->rxbuf[0] == PSP_ADHOC_MATCHING_PACKET_PING)
						actOnPingPacket(context, &sendermac);

					// Hello Packet
					else if (context->rxbuf[0] == PSP_ADHOC_MATCHING_PACKET_HELLO)
						actOnHelloPacket(context, &sendermac, rxbuflen);

					// Join Packet
					else if (context->rxbuf[0] == PSP_ADHOC_MATCHING_PACKET_JOIN)
						actOnJoinPacket(context, &sendermac, rxbuflen);

					// Accept Packet
					else if (context->rxbuf[0] == PSP_ADHOC_MATCHING_PACKET_ACCEPT)
						actOnAcceptPacket(context, &sendermac, rxbuflen);

					// Cancel Packet
					else if (context->rxbuf[0] == PSP_ADHOC_MATCHING_PACKET_CANCEL)
						actOnCancelPacket(context, &sendermac, rxbuflen);

					// Bulk Data Packet
					else if (context->rxbuf[0] == PSP_ADHOC_MATCHING_PACKET_BULK)
						actOnBulkDataPacket(context, &sendermac, rxbuflen);
					...
					// Birth Packet
					else if (context->rxbuf[0] == PSP_ADHOC_MATCHING_PACKET_BIRTH)
						actOnBirthPacket(context, &sendermac, rxbuflen);

					// Death Packet
					else if (context->rxbuf[0] == PSP_ADHOC_MATCHING_PACKET_DEATH)
						actOnDeathPacket(context, &sendermac, rxbuflen);

					// Bye Packet
					else if (context->rxbuf[0] == PSP_ADHOC_MATCHING_PACKET_BYE)
						actOnByePacket(context, &sendermac);
