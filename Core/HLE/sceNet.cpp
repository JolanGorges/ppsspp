...
void netValidateLoopMemory()
{
	...
	if (!apctlThreadHackAddr || (apctlThreadHackAddr && strcmp("apctlThreadHack", kernelMemory.GetBlockTag(apctlThreadHackAddr)) != 0))
	{
		...
		if (apctlThreadHackAddr)
			Memory::Memcpy(apctlThreadHackAddr, apctlThreadCode, sizeof(apctlThreadCode));
	}
}
...
template <typename I>
std::string num2hex(I w, size_t hex_len)
{
...
static int sceNetApctlGetInfo(int code, u32 pInfoAddr)
{
	...
	// if (!netApctlInited)
	//	return hleLogError(Log::sceNet, ERROR_NET_APCTL_NOT_INITIALIZED, "apctl not initialized?"); // FIXME: Official prx files seems to return 0x80410a0d when Apctl not initialized yet?

	// if (netApctlState != PSP_NET_APCTL_STATE_GOT_IP)
	//	return hleLogError(Log::sceNet, ERROR_NET_APCTL_NOT_IN_BSS, "apctl not connected?"); // FIXME: Official prx files seems to return 0x80410a05 when Apctl not connected yet?
...
static u32 sceNetInetInetNtop(int af, u32 srcInAddrPtr, u32 dstBufPtr, u32 bufsize)
{
	...
	if (!Memory::IsValidAddress(dstBufPtr) || bufsize < 1 /*8*/)
	{ // usually 8 or 16, but Coded Arms Contagion is using bufsize = 4
...
int sceNetInetSelect(int nfds, u32 readfdsPtr, u32 writefdsPtr, u32 exceptfdsPtr, u32 timeoutPtr)
{
	...
	if (readfds != NULL && inetLastSocket < maxfd)
		NetInetFD_ZERO(readfds); // Clear it only when not needing a workaround
	if (writefds != NULL && inetLastSocket < maxfd)
		NetInetFD_ZERO(writefds); // Clear it only when not needing a workaround
	if (exceptfds != NULL)
...
static int sceNetInetSetsockopt(int socket, int level, int optname, u32 optvalPtr, int optlen)
{
	...
	else if (level == PSP_NET_INET_SOL_SOCKET && optname == PSP_NET_INET_SO_REUSEPORT)
	{ // 15
	...
	else if (level == PSP_NET_INET_SOL_SOCKET && optname == 0x1022)
	{ // PSP_NET_INET_SO_NOSIGPIPE ?
	...
	else if (level == PSP_NET_INET_SOL_SOCKET && (optname == PSP_NET_INET_SO_RCVBUF || optname == PSP_NET_INET_SO_SNDBUF))
	{ // PSP_NET_INET_SO_NOSIGPIPE ?
...
static int sceNetInetShutdown(int socket, int how)
{
	...
	switch (how)
	{
	case PSP_NET_INET_SHUT_RD:
		hostHow = SHUT_RD;
		break;
	case PSP_NET_INET_SHUT_WR:
		hostHow = SHUT_WR;
		break;
	case PSP_NET_INET_SHUT_RDWR:
		hostHow = SHUT_RDWR;
		break;
	}
...
int NetResolver_StartNtoA(int rid, u32 hostnamePtr, u32 inAddrPtr, int timeout, int retry)
{
	if (netResolvers.find(rid) == netResolvers.end())
		return hleLogError(Log::sceNet, ERROR_NET_RESOLVER_BAD_ID, "Bad Resolver Id: %i", rid);
	...
	if (resolved)
	{
		...
		INFO_LOG(Log::sceNet, "%s - Hostname: %s => IPv4: %s", __FUNCTION__, hostname.c_str(), ip2str(addr.in.sin_addr, false).c_str());
	}
...
{
	WARN_LOG(Log::sceNet, "UNTESTED %s(%d, %08x, %08x, %d, %d) at %08x", __FUNCTION__, rid, hostnamePtr, inAddrPtr, timeout, retry, currentMIPS->pc);
...
{
	ERROR_LOG_REPORT_ONCE(sceNetResolverStartNtoAAsync, Log::sceNet, "UNIMPL %s(%d, %08x, %08x, %d, %d) at %08x", __FUNCTION__, rid, hostnamePtr, inAddrPtr, timeout, retry, currentMIPS->pc);
	return NetResolver_StartNtoA(rid, hostnamePtr, inAddrPtr, timeout, retry);
}
...
{
	ERROR_LOG_REPORT_ONCE(sceNetResolverPollAsync, Log::sceNet, "UNIMPL %s(%d, %08x) at %08x", __FUNCTION__, rid, unknown, currentMIPS->pc);
	return 0;
}
...
{
	ERROR_LOG_REPORT_ONCE(sceNetResolverWaitAsync, Log::sceNet, "UNIMPL %s(%d, %08x) at %08x", __FUNCTION__, rid, unknown, currentMIPS->pc);
	return 0;
}
...
{
	ERROR_LOG_REPORT_ONCE(sceNetResolverStartAtoN, Log::sceNet, "UNIMPL %s(%d, %08x[%s], %08x, %i, %i, %i) at %08x", __FUNCTION__, rid, inAddr, ip2str(*(in_addr *)&inAddr, false).c_str(), hostnamePtr, hostnameLength, timeout, retry, currentMIPS->pc);
	return 0;
}

static int sceNetResolverStartAtoNAsync(int rid, u32 inAddr, u32 hostnamePtr, int hostnameLength, int timeout, int retry)
{
	ERROR_LOG_REPORT_ONCE(sceNetResolverStartAtoNAsync, Log::sceNet, "UNIMPL %s(%d, %08x[%s], %08x, %i, %i, %i) at %08x", __FUNCTION__, rid, inAddr, ip2str(*(in_addr *)&inAddr, false).c_str(), hostnamePtr, hostnameLength, timeout, retry, currentMIPS->pc);
	return 0;
}
...
{
	WARN_LOG(Log::sceNet, "UNTESTED %s(%08x[%d], %08x, %d) at %08x", __FUNCTION__, ridPtr, Memory::Read_U32(ridPtr), bufferPtr, bufferLen, currentMIPS->pc);
	if (!Memory::IsValidRange(ridPtr, 4))
		return hleLogError(Log::sceNet, ERROR_NET_RESOLVER_INVALID_PTR, "Invalid Ptr: %08x", ridPtr);
...
{
	WARN_LOG(Log::sceNet, "UNTESTED %s(%d) at %08x", __FUNCTION__, rid, currentMIPS->pc);
	if (netResolvers.find(rid) == netResolvers.end())
		return hleLogError(Log::sceNet, ERROR_NET_RESOLVER_BAD_ID, "Bad Resolver Id: %i", rid);

	auto &resolver = netResolvers[rid];
	if (!resolver.isRunning)
		return hleLogError(Log::sceNet, ERROR_NET_RESOLVER_ALREADY_STOPPED, "Resolver Already Stopped (Id: %i)", rid);
...
{
	ERROR_LOG(Log::sceNet, "UNIMPL sceNetUpnpGetNatInfo(%08x)", unkStructPtr);
...
{
	ERROR_LOG(Log::sceNet, "UNIMPL sceNetUpnp_8513C6D1(%08x, %08x, %08x)", unk1, unk2, unk3);
	return 0;
}

static int sceNetUpnp_FDA78483()
{
	ERROR_LOG(Log::sceNet, "UNIMPL sceNetUpnp_FDA78483()");
	return 0;
}

static int sceNetUpnp_1038E77A(u32 unkStructPtr)
{
	ERROR_LOG(Log::sceNet, "UNIMPL sceNetUpnp_1038E77A(%08x)", unkStructPtr);
