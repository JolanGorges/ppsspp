...
void AfterMatchingMipsCall::run(MipsCall &call)
{
	...
	if (__IsInInterrupt())
		ERROR_LOG(Log::sceNet, "AfterMatchingMipsCall::run [ID=%i][Event=%d] is Returning Inside an Interrupt!", contextID, EventID);
...
void AfterAdhocMipsCall::run(MipsCall &call)
{
	u32 v0 = currentMIPS->r[MIPS_REG_V0];
	if (__IsInInterrupt())
		ERROR_LOG(Log::sceNet, "AfterAdhocMipsCall::run [ID=%i][Event=%d] is Returning Inside an Interrupt!", HandlerID, EventID);
...
int friendFinder()
{
	...
	while (friendFinderRunning)
	{
		...
		if (networkInited)
		{
			...
			{ // We may need to use lower interval to prevent getting timeout at Pro Adhoc Server through internet
				// Prepare Packet
...
{				  // opt = SO_RCVBUF/SO_SNDBUF
	...
	case SO_RCVBUF:
		n = std::max(size, 128);
		break; // FIXME: The minimum (doubled) value for SO_RCVBUF is 256 ? (2048+MTU+padding on newer OS? TCP_SKB_MIN_TRUESIZE)
	case SO_SNDBUF:
		n = std::max(size, 1024);
		break; // FIXME: The minimum (doubled) value for SO_SNDBUF is 2048 ? (twice the minimum of SO_RCVBUF on newer OS? TCP_SKB_MIN_TRUESIZE * 2)
	}
...
{ // opt = SO_SNDTIMEO/SO_RCVTIMEO
	if (timeout_usec > 0 && timeout_usec < minSocketTimeoutUS)
		timeout_usec = minSocketTimeoutUS; // Override timeout for high latency multiplayer
#if defined(_WIN32)
	unsigned long optval = timeout_usec / 1000UL;
	if (timeout_usec > 0 && optval == 0)
		optval = 1; // Since there are games that use 100 usec timeout, we should set it to minimum value on Windows (1 msec) instead of using 0 (0 = indefinitely timeout)
...
int initNetwork(SceNetAdhocctlAdhocId *adhoc_id)
{
	...
	if (parameter.channel == PSP_SYSTEMPARAM_ADHOC_CHANNEL_AUTOMATIC)
		parameter.channel = defaultWlanChannel; // Faked Active channel to default channel
